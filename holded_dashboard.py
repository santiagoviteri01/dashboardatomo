...
with tab2:
    with st.container():
        ...
        #
        # 1) FORMULARIO DE FILTROS (fecha/rango + cliente)
        #
        with st.form("filtros"):
            ...
            if isinstance(fechas, (tuple, list)) and len(fechas) == 2:
                ...
                sd = ed = fechas
            # Mostrar error si rango inválido
            if ed < sd:
            ...
            cliente_sel = st.selectbox(
                "🧍‍♂️ Selecciona Cliente (solo impacta métricas arriba)",
                opciones_cliente,
                index=opciones_cliente.index(st.session_state.get("cliente", "Todos"))
            )
            # Botón de envío
            filtros_ok = st.form_submit_button("🔄 Actualizar")

        # Detener flujo hasta pulsar "Actualizar"
        if not filtros_ok:
            st.stop()
        ...
        #
        # 2) CALCULAR df_range Y MOSTRAR MÉTRICAS / GRÁFICOS
        #
        ...
        if start_date == end_date:
            ...
            # Altas por día
            df_altas = consultar(f"""
                SELECT DATE(ts_creation) AS fecha,
                       COUNT(*) AS nuevas_altas
                FROM plasma_core.users
        ...
        # Mostrar gráficos (sin zoom) con puntos
        for col in df_range.columns:
            ...
            df_plot = (
                df_range[[col]]
                .reset_index()
                .rename(columns={"index": "Fecha", col: title})
            )
            chart = (
                alt.Chart(df_plot)
        ...
        #
        # 3) FORMULARIO “Top 20 Clientes por KPI”
        #
        st.markdown("---")
        st.header("🔎 Top 20 Clientes por KPI")
        with st.form("top20"):
            # Ya tenemos fechas en session_state
            start_str = st.session_state["fechas"][0].strftime("%Y-%m-%d")
            end_str   = st.session_state["fechas"][1].strftime("%Y-%m-%d")
            ...
            kpi_map = {
                ...
                '💰 Depósitos (Transacciones)': (
                    "COUNT(*)",
                    "(SELECT user_id, ts_commit FROM plasma_payments.nico_transactions WHERE ts_commit BETWEEN "
                    f"'{start_str} 00:00:00' AND '{end_str} 23:59:59' "
                    "UNION ALL "
                    "SELECT user_id, ts_commit FROM plasma_payments.payphone_transactions "
                    f"WHERE ts_commit BETWEEN '{start_str} 00:00:00' AND '{end_str} 23:59:59'"
                    ") t",
                    "t.ts_commit IS NOT NULL",
                    "t.user_id"
                ),
                '🎮 Jugadores': (
                    "COUNT(DISTINCT re.session_id)",
                    "plasma_games.rounds_entries re JOIN plasma_games.sessions s ON re.session_id=s.session_id",
            ...
            kpi_sel = st.selectbox("📊 Selecciona KPI", list(kpi_map.keys()), key="det_kpi")
            top20_ok = st.form_submit_button("Mostrar Top 20")

        if top20_ok:
            agg, from_clause, where_clause, group_col = kpi_map[kpi_sel]
            sql = f"""
                SELECT {group_col} AS user_id,
                       {agg} AS valor
                FROM {from_clause}
                WHERE {where_clause}
                GROUP BY {group_col}
